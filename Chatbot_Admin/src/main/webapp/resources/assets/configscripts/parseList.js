...
function next() {
    var haveData = 0;
    if ((count <= 3)) {
        if ((flagClick != 0) || (indexComplete[currentPosition] != "")||(count == 2)) {
            if (flagClick == 2) {
            ...
            if (indexComplete[currentPosition] != "" && currentPosition < 3 && flagClickNew != 1 && flagEditXPath != 1) {
...
function backProgressBar() {
    ...
    $('.progressRecipe .circle:nth-of-type(' + (step - 1) + ') .labelRecipe').html(step - 3);
    $('.progressRecipe .circle:nth-of-type(' + (step - 1) + ')').addClass('active').removeClass('done');
    if (indexComplete[step - 3] != "") {
    ...
    for (var i = 0; i < cells.length - 1; i++) {
        ...
        if (type == 1) {
            ...
            switch (count) {
                case 1:
                    newCell.innerHTML =
                            '<input type="hidden" name="PAGE" value="' + cells[i] + '" size="78"/>';
                    break;
                case 2:
                    newCell.innerHTML =
                            '<input type="hidden" name="FOODNAME" value="' + cells[i] + '" size="78"/>';
                    break;
                case 3:
                    newCell.innerHTML =
...
function appendcontents(item) {
    ...
    var content = '<h3>Information</h3><br/><table border="1" style="width: 485px" class="table"><thead></thead><tr><th style="width: 10%">Type</th><th style="width: 90%">Content</th></tr><tbody>'
            + '<tr><td><strong>Xpath</strong></td><td>';
    //get Name
    ...
    content = content + alertText + '</td></tr><tr><td><strong>Food Name</strong></td><td>';
